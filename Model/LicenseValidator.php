...
/**
 * Portal license validator for ETechFlow_OrderEmailEditor.
 * Follows PORTAL_LICENSING_GUIDE.md §3-step-1.
 *
 *   isValid() priority:
 *     1. revoked = 1                       → false (portal revoke wins even in dev)
 *     2. production_environment = No       → true (dev bypass — currently disabled)
 ...
 *   removes a server's IP from the portal subscription, /license/validate returns
 *   HTTP 403 with valid:false, which counts as an "explicit reject" and locks the
 *   module immediately. Grace only applies when the portal literally cannot be
 *   reached (timeout, network error, no URL).
 */
class LicenseValidator
{
    ...
    public const XML_PATH_PORTAL_API_URL         = 'etechflow_orderemaileditor/license/portal_api_url';
    ...
    private function getPortalApiBase(): string
    {
        ...
        if ($api !== '' && !str_contains($api, '127.0.0.1') && !str_contains($api, 'localhost')) {
            return $api;
        }
        $browser = trim((string) $this->scopeConfig->getValue(self::XML_PATH_PORTAL_URL));
        if ($browser === '') {
            $browser = self::DEFAULT_PORTAL_URL;
        }
        if ($browser !== '' && !str_contains($browser, '127.0.0.1') && !str_contains($browser, 'localhost')) {
            // Strip the endpoint suffix so validateViaPortal can append it cleanly.
            return (string) preg_replace('#/license/validate/?$#', '', $browser);
        }
        return '';
    }
    ...
    /**
     * The customer-facing portal endpoint (full /license/validate URL). Used by
     * the admin licensing controllers to build browser links; not consulted by
     * isValid(), which goes through getPortalApiBase().
     */
     ...
     * dev-bypass / bundle keys / unreachable portal (full features, no
     * restriction). Feature gating only ever turns OPTIONAL behaviour on; it is
     * never a licensing bypass, and defaults to fully-enabled.
     *
    ...
    public function getPlanFeatures(): array
    {
        ...
        return $this->fetchPortalFeatures($host, $key);
    }
    ...
    /**
     * Fetch the plan feature map from the portal. Cached briefly; returns [] on
     * any non-200 / unreachable response so a network blip never silently
     * disables optional features.
     *
     * @return array<string,mixed>
     */
    private function fetchPortalFeatures(string $host, string $key): array
    {
        $cacheKey = self::CACHE_PREFIX . 'feat_' . md5($host . ':' . $key);
        $cached   = $this->cache->load($cacheKey);
        ...
        $this->cache->save(json_encode($features), $cacheKey, [self::CACHE_TAG], self::CACHE_TTL_VALID);
        return $features;
    }
    ...
    private function isExplicitlyRevoked(): bool
    {
        return (string) $this->scopeConfig->getValue(
            'etechflow_orderemaileditor/license/revoked',
